...
RSpec.describe WeatherServiceClient, type: :service do
  ...
  describe '#current_weather' do
    subject { weather_service_client.current_weather(lat: 40.7128, lon: -74.0060, lang: :en, units: :imperial) }

    it 'calls current_weather on the OpenWeather client' do
      expect(open_weather).to receive(:current_weather).with(
        lat: 40.7128,
        lon: -74.0060,
        lang: :en,
        units: :imperial
    ...
    it 'calls open_weather only once and caches the instance' do
      ...
      2.times { weather_service_client.current_weather(lat: 40.7128, lon: -74.0060, lang: :en, units: :imperial) }
    end
  end
